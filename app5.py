...
# y_accumulated = np.empty_like(y_train[:0])
        ...
        if ANS == 'False':
            ...
            XCnn_train, XCnn_test, y_train, y_test = train_test_split(XCnn, y, test_size=test_size, random_state=random_state)
            ...
            image = image.flatten().reshape(1, 1, 28, 28)
            image_tensor = torch.FloatTensor(image)


            # Make predictions using the provided model (net)
            prediction = cnn.predict(image_tensor)
